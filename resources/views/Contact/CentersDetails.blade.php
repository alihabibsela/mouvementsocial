...
<div class="curved-div" style="width:100%;">
    <div hidden class="curvedBackgroundImage" style="background: url('uploads/images/{{$pages->banners[0]->image}}');">
...
<section class="cause-details blog-details  pt-5 pb-5">
    <div class="container">
        ...
        <?php
            $title=$Centers->name_en;
            $details=$Centers->text_en;
            $address=$Centers->address_en;
            if($_SESSION["lng"]=="ar"){
                $title=$Centers->name_ar;
                $details=$Centers->text_ar;
                $address=$Centers->address_en;
            }else if($_SESSION["lng"]=="fr"){
                $title=$Centers->name_fr;
                $details=$Centers->text_fr;
                $address=$Centers->address_fr;
            }
        ...
        <div class="row">
            <div class="col-md-12 col-lg-8 mx-auto">
                <div class="cause-details__content">
                    <p><?=$address?></p>
                    <p <?=empty($Centers->phone)?"style=display:none;":""?>><?=$_SESSION["lng"]=="ar"?"هاتف":($_SESSION["lng"]=="fr"?"Téléphone":"Phone")?>: <?=$Centers->phone?></p>
                    <p <?=empty($Centers->fax)?"style=display:none;":""?>><?=$_SESSION["lng"]=="ar"?"فاكس":($_SESSION["lng"]=="fr"?"Fax":"Fax")?>: <?=$Centers->fax?></p>
                    <p <?=empty($Centers->email)?"style=display:none;":""?>><?=$_SESSION["lng"]=="ar"?"البريد الإلكتروني":($_SESSION["lng"]=="fr"?"Email":"Email")?>: <?=$Centers->email?></p>
                    <p><?=$details?></p>
                </div>
            </div>
        </div>
...
<section class="cause-details blog-details" style="{{sizeof($galleryItem)>0?"":"display: none;"}}">
    <div class="container">
        <div class="row">
            <div class="col-md-12 col-lg-12 mx-auto">
                <h3 class="blog-details__title text-center mb-3"><?=$_SESSION["lng"]=="ar"?"الصور":($_SESSION["lng"]=="fr"?"Photos":"Photos")?></h3>
            </div>
                ...
                <div class="comment-one">
                    @foreach ($galleryItem as $item)
                            <?php
                                if(empty($item->video)){
                            ?>
                    <div class="item GalleryItem"  video="" style="margin: 15px;">
                        <img src="uploads/images/{{$item->image}}" style="width: 90%;height: 258px;margin: auto;">
                        <a href="uploads/images/{{$item->image}}" style="display: none;" class="img-popup" aria-label="open image"></a>
                    </div>
                    <?php }?>
                    @endforeach
...
<section class="cause-details blog-details mb-5" style="{{sizeof($galleryItem)>0?"":"display: none;"}}">
    <div class="container">
        <div class="row">
            <div class="col-md-12 col-lg-12 mx-auto">
                <h3 class="blog-details__title text-center mb-3"><?=$_SESSION["lng"]=="ar"?"فيديوهات":($_SESSION["lng"]=="fr"?"Vidéos":"Videos")?></h3>
            </div>
            <div class="col-md-12 col-12 mx-auto">
                <div class="comment-one">
                    @foreach ($galleryItemVideos as $item)
                            <?php
                             if($item->video!=""){
                            $url = $item->video;
                            $urlArray=[];
                                $urlArray = explode("/",$item->video);
                        ...
                        ?>
                        <div class="item GalleryItem" video="{{$url}}" style="margin: 15px;">
                            <img src="uploads/images/{{$item->image}}" style="width: 90%;height: 258px;margin: auto;">
                            {{-- <a href="uploads/images/{{$item->image}}" style="display: none;" class="img-popup" aria-label="open image"></a> --}}
                        </div>
                        <?php }?>
